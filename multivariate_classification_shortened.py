...
def build_data(selected_participants, return_raw_scores=True, bin_edges=None, labels=None):
	...
	if not return_raw_scores:
		...
		for y_to_return in y:
			...
			elif (y_to_return >= 25) and (y_to_return < 55):
				ys_to_return.append(class_labels[1])
			else:
				ys_to_return.append(class_labels[2])
...
df.to_csv(BASE_PATH + 'standard_scaler_results_with_ridge_for_classification_without_pca.csv')
...
df.to_csv(BASE_PATH + 'min_max_scaler_results_with_ridge_for_classification_without_pca.csv')