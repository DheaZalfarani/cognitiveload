...
BASE_DIR = 'C:\\Users\\studi\\Documents\\StreamSense\\'
MODEL_BASE_PATH = 'C:\\Users\\studi\\Desktop\\E-Learning\\materials\\'
MODEL_PATH = '2nd_sessions_required\\REPLACE_WITH_MODEL_FILE_FOR_INSTANCE_DT.pkl'
...
def trim_data_to_length_of_experiment(data, experiment_start, experiment_end):
    ...
    for timestamp in data['Timestamp']:
        if datetime.datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S') >= experiment_start: # .%f
            print('WUHUU! IS GREATER! OVERALL, %d ROWS IN THE BEGINNING WERE WRITTEN BEFORE THE START OF THE FIRST TASK!' % start_rows_skipped_ctr)
            break
        else:
    ...
    for timestamp in data.iloc[::-1]['Timestamp']:
        if datetime.datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S') <= experiment_end: # .%f
            print('WUHUU! IS SMALLER! OVERALL, %d ROWS AT THE END WERE WRITTEN AFTER FINISHING THE LAST TASK!' % end_rows_skipped_ctr)
            break
        else:
    ...
    return data[data['Timestamp'].between(experiment_start, experiment_end)] # data.iloc[:,start_rows_skipped_ctr:-end_rows_skipped_ctr]
...
def get_label_for_timestamp(timestamp_str, labels):
    timestamp = timestamp_str.iloc[0] # datetime.datetime.strptime(timestamp_str, '%Y-%m-%d %H:%M:%S.%f')
...
def extract_eeg_features(window):
    window = pd.DataFrame({'AF7':window[0], 'AF8':window[1], 'TP9':window[2], 'TP10':window[3]})
    EEG_FS = 256
...
def bvp_to_hr(input_df):
    input_df = pd.DataFrame({'Timestamp':input_df[0], 'BVP':input_df[1]})
    input_df['BVP'] = input_df['BVP'].str[:4]
...
def extract_gsr_features(window):
    window = pd.DataFrame({'GSR':window[1]})
    window['GSR'] = window['GSR'].str[:3]
...
def extract_temp_features(window):
    window = pd.DataFrame({'TEMP':window[1]})
    window['TEMP'] = window['TEMP'].str[:3]
...
def extract_features_for_windows(sample_to_classify):
    tmp_eeg_features = extract_eeg_features(sample_to_classify["EEG"])
    tmp_bvp_features = extract_bvp_features(sample_to_classify["BVP"])
    tmp_gsr_features = extract_gsr_features(sample_to_classify["GSR"])
    tmp_temp_features = extract_temp_features(sample_to_classify["TEMP"])
    return np.concatenate((tmp_eeg_features, tmp_bvp_features, tmp_gsr_features, tmp_temp_features), axis=None)


def get_creation_time(item):
    item_path = join(BASE_DIR, item)
    return getctime(item_path)


if __name__ == '__main__':
    # Had to run on windows system, as the Empatica E4 streaming server application was available only for this platform...
    stream_runs_items = listdir(BASE_DIR)
    sorted_items = sorted(stream_runs_items, key=get_creation_time)
    data_folder = 'C:\\Users\\studi\\Documents\\StreamSense\\%s\\RawData\\' % sorted_items[-1]   # MAKE SURE TO ALWAYS READ FROM THE LASTLY CREATED FILE!!
    stream_files = [join(data_folder, f) for f in listdir(data_folder) if isfile(join(data_folder, f))]
    stream_ids = [filename.split('.')[0].split('_')[-1] for filename in stream_files]

    # This code runs with assumed quasi-constant sampling frequencies, which is not how it actually happens during streaming
    # Hence, the computations are slightly off from the theoretical optimum/reality, but for the sake of this implementation we'll ignore this
    stream_sample_rates = {
        "EEG": 256,
        "BVP": 64,
        "PPG": 64,
    ...
    # Run this code infinitely and just stop it with CTRL+C once the whole experiment is done
    last_run_to_check_timestamp_with_buffer_time = datetime.datetime.now() + datetime.timedelta(minutes=1) # datetime.timedelta(seconds=40)     # make sure that the script doesn't start too early and there's not enough data
    while True:
        if datetime.datetime.now() >= last_run_to_check_timestamp_with_buffer_time:
            # Run the data reader, only caring about EEG, BVP, PPG, GSR, TEMP
            sample_to_classify = {
                "EEG": None,
                "BVP": None,
                "PPG": None,
            ...
            for idx, filepath in enumerate(stream_files):
                stream_id = filepath.split('.')[0].split('_')[-1]
                if ('.csv' in filepath) and (('EEG' in stream_id) or ('BVP' in stream_id) or ('PPG' in stream_id) or ('GSR' in stream_id) or ('TEMP' in stream_id)):
                    print('NOW WORKING ON %s which is a file of stream type %s' % (filepath, stream_id))
                    try:
                        with open(filepath, 'rb') as csv_file:
                            last_lines_np = np.asarray([[elem.rstrip() for elem in str(line).split(',')[1:5]] for line in csv_file.readlines()[-(stream_sample_rates[stream_id] * 30):]])   # Reading last thirty seconds to classify the Cognitive Load
                            sample_to_classify[stream_id] = last_lines_np
                    except Exception as e:
                        print(f"Error processing {filepath}: {e}. Traceback: {traceback.format_exc()}")
                        print('Will simply wait for a brief while to retry...')
                        time.sleep(2)   # We don't need frequent checks on when the data can be polled, and we don't really care too much about seconds-level precision

            clf2 = joblib.load(MODEL_BASE_PATH + MODEL_PATH)
            X = extract_features_for_windows(sample_to_classify)
            X[np.isnan(X)] = 0
            predicted_label = clf2.predict(X.reshape(1, -1))
            ...
            if os.path.exists(adjustment_flag_file):
                with open(adjustment_flag_file, 'w') as filetowrite:
                    ...
                    if prediction == 3:
                        prediction = 1
                    filetowrite.write(str(prediction))  # Code 0 == No adjustment needed!; Code 1 == Speedup!; Code -1 == Slow Down!

            last_run_to_check_timestamp_with_buffer_time = datetime.datetime.now() + datetime.timedelta(seconds=30) # + datetime.timedelta(seconds=30)
        else:
            time.sleep(10)   # We don't need frequent checks on when the data can be polled, and we don't really care too much about seconds-level precision